...
# se lee el archivo env para tener las credenciales de conexion
...
with engine.connect() as connection:
    ...
    output_file = 'pedidos_cancelados.csv'
    df.to_csv(output_file, encoding='utf-8', index=False)
    print(f'Se ha creado el archivo {output_file} correctamente')
...
# ID de la carpeta de Google Drive a donde subirás el archivo
FOLDER_ID = '1SJqdqRtlNRu0ThgPEEMhslMcqPowehK-'

# Verifica que la carpeta es accesible por la cuenta de servicio
try:
    drive_service.files().get(fileId=FOLDER_ID).execute()
    print("✅ La cuenta de servicio puede acceder a la carpeta.")
except Exception as e:
    print("❌ No se puede acceder a la carpeta. Verifica el ID o permisos.")
    raise e
...
# Obtener ID del archivo y generar link
file_id = file.get('id')
print(f"Archivo subido con ID: {file_id}")
print(f"URL descarga directa: https://drive.google.com/uc?export=download&id={file_id}")

# --- Tiempo total ---
end_time = timeit.default_timer()
duration = end_time - start_time
print(f'Duración total del programa: {duration:.2f} segundos')
